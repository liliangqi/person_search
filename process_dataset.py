...
def process_images_mat(root_dir):
    ...
    bboxes_df.to_csv(osp.join(rtdir, 'bboxesDF.csv'), index=False)
    imnames.to_csv(osp.join(rtdir, 'imnamesSe.csv'), index=False)
...
def process_train_mat(root_dir):
    ...
    train_df.to_csv(osp.join(rtdir, 'trainLabeledDF.csv'), index=False)
...
def process_pool_mat(root_dir):
    ...
    all_imgs = pd.read_csv(osp.join(rtdir, 'imnamesSe.csv'), header=None,
                           squeeze=True)
